...
const maxCharacters = 10; // Maximum characters to display

// Get the content of the username element
const usernameText = usernameElement.textContent.trim();

// If the username text exceeds the maximum characters, truncate and add an ellipsis
if (usernameText.length > maxCharacters) {
    const truncatedText = usernameText.slice(0, maxCharacters) + '...';
    usernameElement.textContent = truncatedText;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const profileSection = document.getElementById('profile-section'); // Replace 'profile-section' with the actual ID of your profile section
